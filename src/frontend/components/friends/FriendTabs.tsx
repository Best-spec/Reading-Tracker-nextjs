...
interface FriendTabsProps {
  ...
  setActiveTab: (tab: 'friends' | 'requests' | 'online') => void
  requestCount: number
  onlineCount: number
}

export function FriendTabs({ activeTab, setActiveTab, requestCount, onlineCount }: FriendTabsProps) {
  const tabs: Array<{ id: 'friends' | 'requests' | 'online'; label: string; count?: number; color?: string }> = [
